...
(() => {
	...
	function FriendStatus(props) {
		...
		useEffect(() => {
			...
			console.log(`${props.friend.id} useEffect skippable start`);

			return function cleanup() {
				ChatAPI.unsubscribeFromFriendStatus(props.friend.id, handleStatusChange);
				
				console.log(`${props.friend.id} useEffect skippable cleanup`);
			};
	...
	ReactDOM.render(
		...
		document.getElementById(`hooks-02-use-effect-skippable`)
	);
})();